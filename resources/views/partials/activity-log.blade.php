<div class="card-panel activity-log-panel">
    <div class="card-panel-header">
        <div class="card-panel-title"><i class="bi bi-shield-check me-2"></i>Recent Activity</div>
            ...
            <div class="activity-log-item {{ $log->severityColorClass() }}">
                ...
                <div class="activity-log-content">
                    ...
                    <div class="activity-log-meta">
                        @if($log->ip_address && in_array($log->action, ['auth.login_failed', 'auth.login']))
                            <span class="activity-log-ip"><i class="bi bi-router me-1"></i>{{ $log->ip_address }}</span>
                        ...
                        <span class="activity-log-time"
                            title="{{ $log->created_at->format('M j, Y H:i:s') }}">{{ $log->created_at->diffForHumans() }}</span>
                    </div>
                </div>
                ...
                @if($log->severity !== 'info')
                    <div class="activity-log-badge">
                        <span
                            class="badge{{ $log->severity === 'danger' ? ' bg-danger' : ' bg-warning text-dark' }}">{{ strtoupper($log->severity) }}</span>
                    </div>
                @endif
            </div>
        @empty
