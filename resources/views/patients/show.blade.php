...
<div class="page-content">
            ...
            <div class="patient-header-info">
                ...
                <div class="patient-meta">
                    ...
                    @if($patient->city || $patient->province)
                        <span><i class="bi bi-geo-alt me-1"></i>{{ collect([$patient->city, $patient->province])->filter()->implode(', ') }}</span>
                    @endif
                </div>
            ...
            <div class="patient-header-actions">
                @if($role === 'secretary')
                    ...
                    <button class="btn btn-outline-warning btn-sm"
                            data-bs-toggle="modal" data-bs-target="#addNotesModal">
    ...
    <div class="card-panel mb-4" style="border-top: none; border-radius: 0 0 12px 12px; margin-top: -8px;">
        <div class="patient-info-toggle"
            ...
            <i class="bi bi-chevron-down patient-info-chevron text-muted" style="font-size:13px; transition: transform 0.2s;"></i>
        </div>
            ...
            <div class="row g-3" style="font-size:13.5px;">
                ...
                <div class="col-md-4">
                    ...
                    <div class="patient-info-row align-items-start">
                        <span class="patient-info-key"><i class="bi bi-exclamation-triangle me-1 text-danger"></i>Allergies</span>
                    ...
                    <div class="patient-info-row align-items-start mt-2">
                        <span class="patient-info-key" style="min-width:unset"><i class="bi bi-clipboard2-pulse me-1"></i>Medical & Vaccine History</span>
                    </div>
                    <div class="text-muted mt-1" style="font-size:12.5px; white-space:pre-line; line-height:1.6;">{{ $patient->medical_history }}</div>
                    @else
                    <div class="patient-info-row">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div class="d-flex gap-2 align-items-center" style="font-size:11px;color:#64748b">
                            <span class="d-flex align-items-center gap-1">
                                <span style="width:10px;height:10px;background:#f59e0b;border-radius:2px;display:inline-block"></span>Pending
                            </span>
                            <span class="d-flex align-items-center gap-1">
                                <span style="width:10px;height:10px;background:#0ea5e9;border-radius:2px;display:inline-block"></span>In Progress
                            </span>
                            <span class="d-flex align-items-center gap-1">
                                <span style="width:10px;height:10px;background:#22c55e;border-radius:2px;display:inline-block"></span>Completed
                            </span>
                        </div>
                    ...
                    @forelse($patient->visits->sortByDesc('visited_at') as $loop_visit => $visit)
                        ...
                        <div class="visit-timeline-item">
                            ...
                            <div class="visit-timeline-inner">
                                ...
                                <div class="visit-header visit-collapse-trigger"
                                     data-bs-toggle="collapse"
                                     data-bs-target="#visitBody{{ $visit->id }}"
                                     style="cursor:pointer">
                                    <div class="visit-header-left">
                                        <i class="bi bi-chevron-{{ $isFirst ? 'up' : 'down' }} visit-chevron me-1 text-muted" style="font-size:11px"></i>
                                ...
                                <div class="collapse {{ $isFirst ? 'show' : '' }}" id="visitBody{{ $visit->id }}">
                                    ...
                                    @if($visit->blood_pressure || $visit->weight || $visit->height)
                                        <div class="visit-vitals mt-2 d-flex flex-wrap gap-2">
                                            @if($visit->blood_pressure)
                                                <span class="vital-pill"><i class="bi bi-activity me-1"></i>BP: {{ $visit->blood_pressure }}</span>
                                            @endif
                                            @if($visit->weight)
                                                <span class="vital-pill"><i class="bi bi-moisture me-1"></i>Wt: {{ $visit->weight }} kg</span>
                                            @endif
                                            @if($visit->height)
                                                <span class="vital-pill"><i class="bi bi-arrows-vertical me-1"></i>Ht: {{ $visit->height }} cm</span>
                                            @endif
                                        </div>
                                    @endif
                                    ...
                                    @if($role === 'doctor' || $visit->isFinalized())
                                        ...
                                        @if($visit->plan_instructions)
                                            <div class="visit-section mt-2">
                                                <div class="visit-field-label"><i class="bi bi-journal-text me-1"></i>Instructions</div>
                    ...
                    @forelse($patient->appointments->take(6) as $appt)
                        <div class="dash-appt-item">
                            <div class="dash-appt-time">
                                ...
                                <small>{{ $appt->scheduled_at->format('M j') }}</small>
                            </div>
                        ...
                        @foreach($notesList as $note)
                            <div class="patient-note-item">
                                <div class="patient-note-meta">
                                    ...
                                    <span class="text-muted">{{ \Carbon\Carbon::parse($note['created_at'])->format('M j, Y') }}</span>
                                </div>
            ...
            <form method="POST" action="{{ route("{$role}.patients.note", $patient) }}">
                ...
                <div class="modal-body">
                    @if($patient->notes && is_array(json_decode($patient->notes, true)))
                        ...
                        <div class="mb-3 p-3 rounded" style="background:#f8fafc;border:1px solid #e2e8f0;max-height:160px;overflow-y:auto">
                            <div class="text-muted small fw-semibold mb-2">Previous Notes</div>
                            @foreach($existing as $n)
                                <div class="mb-2 pb-2 border-bottom">
                                    <div style="font-size:11px;color:#94a3b8">
                                        {{ $n['author'] ?? '' }} · {{ \Carbon\Carbon::parse($n['created_at'])->format('M j, Y H:i') }}
                                    </div>
                    ...
                    <textarea name="note_text" class="form-control" rows="4"
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('secretary.visits.store', $patient) }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Assign Doctor <span class="text-danger">*</span></label>
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" name="chief_complaint_other" id="intakeCcOther"
                                   class="form-control mt-2" placeholder="Describe complaint..."
                                   style="display:none">
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                <input type="number" name="weight" class="form-control" placeholder="60" step="0.1" min="1" max="500">
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                <input type="number" name="height" class="form-control" placeholder="160" step="0.1" min="30" max="250">
...
<div class="modal fade" id="addVisitModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <form id="dvVisitForm" method="POST"
                  action="{{ route('doctor.visits.store', $patient) }}"
                        ...
                        <div class="tab-pane fade show active" id="dvSubjective">
                            <div class="row g-3">
                                ...
                                <div class="col-md-5">
                                    ...
                                    <div id="dvEntManualNote" class="text-muted mt-1" style="font-size:11px;display:none">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-lg-7">
                                    ...
                                    <div class="row g-2 align-items-end mb-3">
                                        <div class="col-5">
                                            <input type="text" id="dvRxDrug" class="form-control form-control-sm" placeholder="Drug name">
                                        </div>
                                        <div class="col-4">
                                            <input type="text" id="dvRxDosage" class="form-control form-control-sm" placeholder="Dosage/Sig">
                                        </div>
                                        <div class="col-2">
                                            <input type="number" id="dvRxQty" class="form-control form-control-sm" placeholder="Qty" min="1">
                                        </div>
                                        <div class="col-1">
                                            <button type="button" class="btn btn-outline-primary btn-sm w-100" onclick="dvAddRx()"><i class="bi bi-plus"></i></button>
                                        </div>
                                    </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-outline-secondary" id="dvPrintRxBtn"
                            onclick="dvPrintPrescription()" style="display:none">
...
<div class="modal fade" id="completeVisitModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div id="completeVisitIntakeSummary" class="mx-3 mt-3 alert alert-info d-flex gap-3 align-items-start" style="display:none!important">
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane fade" id="cvObjective">
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                <label class="form-label fw-semibold">Weight</label>
                                <input type="text" id="cvWeightDisplay" class="form-control" readonly style="background:#f8fafc">
                                <div class="text-muted" style="font-size:11px;margin-top:3px">Collected by secretary</div>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label fw-semibold">Height</label>
                                <input type="text" id="cvHeightDisplay" class="form-control" readonly style="background:#f8fafc">
                                <div class="text-muted" style="font-size:11px;margin-top:3px">Collected by secretary</div>
                            </div>
                            <div class="col-12">
                                <label class="form-label fw-semibold">Physical Exam</label>
                    ...
                    <div class="tab-pane fade" id="cvPlan">
                        <div class="row g-3">
                            ...
                            <div class="col-lg-7">
                                ...
                                <div class="row g-2 align-items-end mb-3">
                                    <div class="col-5">
                                        <input type="text" id="cvRxDrug" class="form-control form-control-sm" placeholder="Drug name">
                                    </div>
                                    <div class="col-4">
                                        <input type="text" id="cvRxDosage" class="form-control form-control-sm" placeholder="Dosage/Sig">
                                    </div>
                                    <div class="col-2">
                                        <input type="number" id="cvRxQty" class="form-control form-control-sm" placeholder="Qty" min="1">
                                    </div>
                                    <div class="col-1">
                                        <button type="button" class="btn btn-outline-primary btn-sm w-100" onclick="cvAddRx()">
                                ...
                                <div class="mt-3">
                                    <label class="form-label fw-semibold">Follow-up Date</label>
                                    <input type="date" id="cvFollowUp" class="form-control"
                                           style="max-width:200px"
                                           min="{{ date('Y-m-d', strtotime('+1 day')) }}">
                                </div>
                            </div>
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route("{$role}.appointments.store") }}"
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-light d-flex gap-2 py-2 mb-3" style="font-size:13px;border:1px solid #e2e8f0">
                    ...
                    <div class="mb-1">
                        ...
                        <input type="date" name="scheduled_at" id="apptDatetime"
...
.patient-info-val { color: #1e293b; flex: 1; }
.patient-info-toggle:hover { opacity: 0.8; }
.patient-info-chevron.rotated { transform: rotate(180deg); }
...
// Show/hide Print Rx button based on active tab
document.addEventListener('DOMContentLoaded', () => {
    document.querySelector('[data-bs-target="#dvRx"]')?.addEventListener('shown.bs.tab', () => {
        document.getElementById('dvPrintRxBtn').style.display = 'inline-flex';
    });
...
function openCompleteVisitModal(visit) {
    ...
    if (visit.status === 'pending') {
        fetch(`${base}`, {
            ...
            body: JSON.stringify({ _method: 'PUT', status_only: true,
