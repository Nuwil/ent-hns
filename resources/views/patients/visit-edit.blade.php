...
.rx-checklist-panel { border: 1px solid #e2e8f0; border-radius: 10px; overflow: hidden; }
.rx-checklist-header { background: #f8fafc; padding: 10px 14px; font-size: 12px; font-weight: 700; color: #475569; text-transform: uppercase; letter-spacing: 0.4px; border-bottom: 1px solid #e2e8f0; }
.rx-checklist-body { max-height: 360px; overflow-y: auto; padding: 6px 0; }
.rx-cat-group { margin-bottom: 2px; }
.rx-cat-label { font-size: 10.5px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px; padding: 6px 14px 2px; }
.rx-checklist-item { display: flex; align-items: center; gap: 6px; padding: 5px 14px; transition: background 0.1s; width: 100%; }
...
.rx-item-name { font-size: 12px; font-weight: 600; color: #1e293b; flex: 1; min-width: 80px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.rx-sig-input { width: 120px; flex-shrink: 0; font-size: 11px !important; padding: 2px 6px !important; height: auto !important; color: #475569; border-color: #cbd5e1; border-radius: 5px !important; background: #fff; }
.rx-sig-input:disabled { background: transparent; border-color: transparent; color: #94a3b8; cursor: default; }
.rx-qty-input { width: 50px; flex-shrink: 0; font-size: 11px !important; padding: 2px 4px !important; height: auto !important; border-color: #cbd5e1; border-radius: 5px !important; text-align: center; }
.rx-qty-input:disabled { background: transparent; border-color: transparent; color: #94a3b8; cursor: default; }

/* Added prescription list */
.rx-added-item { display: flex; align-items: center; justify-content: space-between; background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 7px; padding: 7px 12px; margin-bottom: 6px; font-size: 13px; }
.rx-added-info { display: flex; align-items: center; flex-wrap: wrap; gap: 4px; }

/* Prescription list container - scrollable */
.prescription-list-container { max-height: 200px; overflow-y: auto; border: 1px solid #e2e8f0; border-radius: 6px; padding: 8px; background: #fafafa; }
</style>
@endpush
...
<div class="page-content">
    ...
    <ul class="nav nav-tabs mb-0" id="soapTabs">
        <li class="nav-item"><button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tabSubjective"><i class="bi bi-chat-square-text me-1"></i>Subjective</button></li>
        <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabObjective"><i class="bi bi-eye me-1"></i>Objective</button></li>
        <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabAssessment"><i class="bi bi-clipboard2-check me-1"></i>Assessment</button></li>
        <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabPlan"><i class="bi bi-capsule me-1"></i>Plan</button></li>
    </ul>
    ...
    <div class="card-panel" style="border-radius:0 0 12px 12px;border-top:none">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <div class="d-flex gap-2">
                            <select id="ccDropdown" class="form-select" style="max-width:320px" onchange="fillChiefComplaint(this.value)">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-lg-7">
                        ...
                        <div class="row g-2 align-items-end mb-3">
                            <div class="col-5"><input type="text" id="rxDrug" class="form-control form-control-sm" placeholder="Drug name"></div>
                            <div class="col-4"><input type="text" id="rxDosage" class="form-control form-control-sm" placeholder="Dosage/Sig"></div>
                            <div class="col-2"><input type="number" id="rxQty" class="form-control form-control-sm" placeholder="Qty" min="1"></div>
                            <div class="col-1"><button type="button" class="btn btn-outline-primary btn-sm w-100" onclick="addPrescription()"><i class="bi bi-plus"></i></button></div>
                        </div>

                        <label class="form-label fw-semibold">Prescriptions Added <span id="rxCount" class="badge bg-primary ms-1">0</span></label>
                        <div class="prescription-list-container mb-3">
                            <div id="prescriptionList"></div>
                        </div>

                        <label class="form-label fw-semibold"><i class="bi bi-journal-text me-1"></i>Instructions <span class="text-muted fw-normal small">(up to doctor's interpretation)</span></label>
        ...
        <div class="d-flex gap-2 justify-content-end mt-4 pt-3 border-top">
            ...
            <button type="button" class="btn btn-outline-primary" onclick="submitVisit('save')"><i class="bi bi-floppy me-1"></i>Save Progress</button>
            <button type="button" class="btn btn-success" onclick="submitVisit('finalize')"><i class="bi bi-lock me-1"></i>Finalize & Lock Visit</button>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-footer border-0 pt-0">
                ...
                <button type="button" class="btn btn-success" id="confirmFinalizeBtn"><i class="bi bi-lock me-1"></i>Yes, Finalize</button>
            </div>
        </div>
    </div>
</div>
...
function renderPrescriptions() {
    ...
    list.innerHTML = prescriptions.length === 0
        ...
        : prescriptions.map((rx, i) => `<div class="rx-added-item"><div class="rx-added-info"><i class="bi bi-capsule-pill me-1 text-primary"></i><strong>${rx.drug}</strong>${rx.dosage ? '<span class="text-muted ms-1">— ' + rx.dosage + '</span>' : ''}${rx.quantity ? '<span class="badge bg-secondary ms-1">Qty: ' + rx.quantity + '</span>' : ''}</div><button type="button" class="btn btn-sm btn-link text-danger p-0" onclick="removePrescription(${i})"><i class="bi bi-x-circle-fill"></i></button></div>`).join('');
}
...
function buildChecklist(filter = '') {
    ...
    Object.entries(entMedicines).forEach(([cat, meds]) => {
        ...
        filtered.forEach(med => {
            ...
            html += `<div class="rx-checklist-item ${checked ? 'rx-checked' : ''}"><input type="checkbox" class="rx-checkbox" value="${med.name}" data-usual="${med.usual}" ${checked} onchange="toggleChecklistItem(this)"><span class="rx-item-name" title="${med.name}">${med.name}</span><input type="text" class="rx-sig-input form-control form-control-sm" value="${sigVal}" placeholder="Dosage..." ${inputDisabled} oninput="updateSigInline(this,'${safeName}')"><input type="number" class="rx-qty-input form-control form-control-sm" value="${qtyVal}" placeholder="Qty" min="1" ${inputDisabled} oninput="updateQtyInline(this,'${safeName}')"></div>`;
        });
        html += `</div>`;
    });
