@extends('layouts.app')@section('title', 'Secretary Dashboard')@section('page-title', 'Dashboard')
@section('content')
    <div class="page-content">
        ...
        <div class="row g-4 mt-1">
            <div class="col-lg-7">
                <div class="card-panel">
                    <div class="card-panel-header">
                        <div class="card-panel-title"><i class="bi bi-calendar-check me-2"></i>Recent Appointments</div>
                        <span class="badge bg-secondary">{{$recentAppointments->count()}}entries</span>
                    </div>
                    <div class="card-panel-body p-0">@forelse($recentAppointments as $appt)
                        <div class="dash-appt-item"
                            style="padding:12px 16px;border-bottom:1px solid #e2e8f0;display:flex;align-items:center;">
                            ...
                            </div><span class="badge{{$appt->statusBadgeClass()}}"
                                style="flex-shrink:0;">{{$appt->status === 'accepted' ? 'Confirmed' : ucfirst($appt->status)}}</span>
                    </div>@empty<div class="empty-state-sm p-4"><i class="bi bi-calendar-x"></i><span>No
                        appointments</span></div>@endforelse
                    </div>
                </div>
            </div>
            <div class="col-lg-5 d-flex flex-column gap-4">
                <div class="card-panel">
                    <div class="card-panel-header">
                        <div class="card-panel-title"><i class="bi bi-calendar2-day me-2"></i>Today's Schedule</div><span
                            class="text-muted small">{{now()->format('M j,Y')}}</span>
                    </div>
                    <div class="card-panel-body p-0">@forelse($todayAppointments as $appt)
                        <div class="dash-appt-item">
                    ...
                    </div>@empty<div class="empty-state-sm p-4"><i class="bi bi-calendar-x"></i><span>No appointments
                        today</span></div>@endforelse
                    </div>
                </div>
                @if($pendingAppointments->count() > 0)
                    <div class="card-panel">
                        <div class="card-panel-header">
                            <div class="card-panel-title"><i class="bi bi-hourglass-split me-2 text-warning"></i>Awaiting
                                Confirmation</div><span
                                class="badge bg-warning text-dark">{{$pendingAppointments->count()}}</span>
                        </div>
                        <div class="card-panel-body p-0">@foreach($pendingAppointments->take(5) as $appt)
                            <div class="dash-appt-item">
