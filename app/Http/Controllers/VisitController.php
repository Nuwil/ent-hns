...
class VisitController extends Controller
{
    ...
    // ================================================================
    // PRINT PRESCRIPTION — from finalized visit
    // ================================================================
    public function printPrescription(Patient $patient, Visit $visit)
    {
        ...
        return view('patients.prescription_print', compact(
            ...
            'prescriptions', 'instructions', 'followUpDate'
        ));
    }

    // ================================================================
    // PREVIEW PRESCRIPTION — from Add Visit modal (before finalize)
    // ================================================================
    public function previewPrescription(Request $request, Patient $patient)
    {
        $prescriptions = json_decode($request->get('rxdata', '[]'), true) ?? [];
        $instructions  = $request->get('instructions', '');
        $followUpDate  = $request->get('followup', '');
        $doctor        = Auth::user();
        $visit         = null;
