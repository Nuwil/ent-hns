...
class VisitController extends Controller
{
    ...
    public function storeIntake(Request $request, Patient $patient)
    {
        ...
        NotificationHelper::intakeCreated(
            $doctorId,
            $patient->full_name,
    ...
    {
        NotificationHelper::visitFinalized(
            Auth::user()->full_name,
            $patient->full_name,
            $patient->id
        );
    ...
    private function formatVitals(array $data): string
    {
        ...
        if (!empty($data['blood_pressure']))
            $lines[] = "BP: {$data['blood_pressure']}";
        if (!empty($data['heart_rate']))
            $lines[] = "HR: {$data['heart_rate']} bpm";
        if (!empty($data['temperature']))
            $lines[] = "Temp: {$data['temperature']}°C";
        if (!empty($data['weight']))
            $lines[] = "Weight: {$data['weight']} kg";
        if (!empty($data['allergies_note']))
            $lines[] = "Allergies: {$data['allergies_note']}";
        if (!empty($data['intake_notes']))
            $lines[] = "Notes: {$data['intake_notes']}";
    ...
    {
        // Only allow finalized visits or doctor's own in-progress
        $prescriptions = $visit->prescriptions ?? [];
        $doctor = $visit->doctor;
        $instructions = $visit->plan_instructions;
        $followUpDate = $visit->follow_up_date
            ? $visit->follow_up_date->format('m/d/Y')
            : null;

        return view('patients.prescription_print', compact(
            'patient',
            'visit',
            'doctor',
            'prescriptions',
